"""
Japanese Wikipedia dataset tokenization script for NanoGPT.
Downloads Japanese Wikipedia (wikimedia/wikipedia, subset 20231101.ja),
tokenizes it using tiktoken (o200k_base), and saves shards to disk.

Run as:
$ python wikipedia_ja.py
Will save shards to the local directory "wiki_ja".
"""
...
with mp.Pool(nprocs) as pool:
    ...
    for tokens in pool.imap(tokenize, dataset, chunksize=16):
        ...
        if token_count + len(tokens) < shard_size:
            ...
            filename = os.path.join(DATA_CACHE_DIR, f"wiki_ja_{split}_{shard_index:06d}")
